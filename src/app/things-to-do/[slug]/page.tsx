...
export async function generateMetadata({params}:{params:Promise<{slug:string}>}): Promise<Metadata>{ const {slug}=await params; const guide=getLocationGuideBySlug(slug); if(!guide) return {}; return { title: 'Things to Do in '+guide.name+' | Lincs Staycation Guide', description: 'Research-led things to do guide for '+guide.name+', Lincolnshire', alternates:{canonical:'/things-to-do/'+guide.slug} }; }
