...
export async function generateMetadata({params}:{params:Promise<{slug:string}>}): Promise<Metadata>{ const {slug}=await params; const guide=getLocationGuideBySlug(slug); if(!guide) return {}; return { title: 'Things to Do in '+guide.name+' | Lincs Staycation Guide', description: 'Discover things to do in '+guide.name+', Lincolnshire', alternates:{canonical:'/things-to-do/'+guide.slug} }; }
