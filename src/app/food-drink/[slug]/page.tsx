...
export async function generateMetadata({params}:{params:Promise<{slug:string}>}): Promise<Metadata>{ const {slug}=await params; const guide=getLocationGuideBySlug(slug); if(!guide) return {}; return { title: 'Food & Drink in '+guide.name+' | Lincs Staycation Guide', description: 'Research-led food & drink guide for '+guide.name+', Lincolnshire', alternates:{canonical:'/food-drink/'+guide.slug} }; }
