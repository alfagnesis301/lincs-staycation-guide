...
export async function generateMetadata({params}:{params:Promise<{slug:string}>}): Promise<Metadata>{ const {slug}=await params; const guide=getLocationGuideBySlug(slug); if(!guide) return {}; return { title: 'Food & Drink in '+guide.name, description: 'Find restaurants, pubs and cafés in '+guide.name+', Lincolnshire', alternates:{canonical:'/food-drink/'+guide.slug} }; }
