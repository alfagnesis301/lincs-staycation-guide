...
export async function generateMetadata({params}:{params:Promise<{slug:string}>}): Promise<Metadata>{ const {slug}=await params; const guide=getLocationGuideBySlug(slug); if(!guide) return {}; const title = 'Places to Stay in '+guide.name; const description = 'Research-led places to stay guide for '+guide.name+', Lincolnshire'; const path = '/places-to-stay/'+guide.slug; return { metadataBase: new URL(SITE_URL), title, description, alternates:{canonical:path}, openGraph:{title: `${title} | Lincs Staycation Guide`, description, url:`${SITE_URL}${path}`, siteName:'Lincs Staycation Guide', locale:'en_GB', type:'article'}, twitter:{card:'summary_large_image', title, description} }; }
